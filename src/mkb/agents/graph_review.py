...
def build_graph_review_agent(
    mode: Literal["global", "local"], runtime: AgentRuntime, model: str | None = None
) -> Agent:
    """Create the graph review agent for the given mode."""
    llm = create_llm(model)
    tools = graph_review_tools(mode, runtime)
...
async def _run_graph_review_async(
    ...
    if runtime is None:
        raise ValueError("Graph review requires an explicit AgentRuntime")
    ...
    if mode == "local":
        seed_labels = _get_least_examined_concepts(
            global_space.space_id, seed_count, runtime=runtime
        )
        if not seed_labels:
            return {
                "mode": mode,
    ...
    agent = build_graph_review_agent(mode, runtime, model)
    ...
    stats = _flush_review_session_to_db(
        global_space.space_id, review_session, runtime=runtime
    )
