...
import { JOB_FINISHED_EVENT, isJobTerminal } from '../../api/jobPolling'
...
export default function ProjectionsPage() {
  ...
  const [reviewJobIds, setReviewJobIds] = useState<string[]>([])
  const reviewJobs = useJobsStore(useShallow(state => reviewJobIds
  ...
  useEffect(() => {
    if (!isReviewing || reviewJobs.length !== reviewJobIds.length || reviewJobs.some(job => !isJobTerminal(job.status))) return
    setIsReviewing(false)
    loadProjections()
  }, [isReviewing, loadProjections, reviewJobIds.length, reviewJobs])
  ...
  const startReview = async (overrideProjectionIds?: string[]) => {
    try {
      ...
      const queuedJobs = jobIds.map(jobId => ({
        job_id: jobId,
      ...
      queuedJobs.forEach(job => useJobsStore.getState().upsertJob(job))
      setReviewJobIds(jobIds)
