...
import { cancelAllJobs, cancelJob, getJob, sendReviewJobChat } from '../api/jobs'
...
function JobDetailPanel({
  ...
  job: Job | null
  onClose: () => void
  onFollowupStarted: (jobId: string) => void
}) {
  ...
  const handleSend = async () => {
    ...
    setError(null)
    try {
      const result = await sendReviewJobChat(job.job_id, text)
      try {
        const nextJob = await getJob(result.job_id)
        useJobsStore.getState().upsertJob(nextJob)
      } catch {}
      setMessage('')
      onFollowupStarted(result.job_id)
    } catch (err) {
      const e = err as { message?: string }
      setError(e?.message ?? 'Could not start review follow-up.')
    } finally {
      setSending(false)
    }
  }
...
{JSON.stringify(job.result, null, 2)}
              ...
              {canChat && (
                <div>
                  <h4 className="text-xs font-semibold uppercase tracking-wide text-slate-400">Follow-up</h4>
                  <textarea
                    value={message}
                    onChange={event => setMessage(event.target.value)}
                    rows={4}
                    className="mt-2 w-full resize-none rounded border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-200 placeholder-slate-500 outline-none transition-colors focus:border-teal-500"
                    placeholder="Ask the reviewer to verify, fill, or patch specific fields."
                  />
                  ...
                  <button
                    ...
                    className="mt-2 w-full rounded bg-teal-700 px-3 py-2 text-xs font-medium text-teal-50 transition-colors hover:bg-teal-600 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {sending ? 'Starting…' : 'Send to reviewer'}
                  </button>
                </div>
              )}
            </div>
  ...
  return (
    <div className="flex-shrink-0 border-t border-slate-700">
      ...
      <JobDetailPanel
        ...
        onFollowupStarted={setSelectedJobId}
      />
    </div>
  )
}
